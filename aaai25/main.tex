...
be at least some value $\delta \in (0,1]$, where $\vz$ is a random
  instance compatible with $\vx$.  Computing small $\delta$-sufficient reasons is known to be notoriously hard; even to approximate them over decision trees.
 We study the problem of computing minimum
  $\delta$-sufficient reasons for linear models, for which we provide a positive theoretical result: a natural relaxation of $\delta$-sufficient reasons that is hard even for decision trees can be solved efficiently over linear models.
\end{abstract}
